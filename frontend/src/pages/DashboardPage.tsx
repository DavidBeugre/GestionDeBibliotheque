...
function ReaderDashboard() {
  ...
  const latestBooksQuery = useQuery({ queryKey: ['books', 'reader-latest'], queryFn: () => bookService.list({ page: 1, limit: 4, status: 'ACTIVE', sort: 'createdAt', order: 'desc' }) });
  ...
  const downloadCardMutation = useMutation({
    mutationFn: memberPortalService.downloadCardPdf,
    onError: (error) => toast.error(getApiErrorMessage(error, 'Impossible de télécharger votre carte')),
  });
  ...
  const addDueDateToCalendar = (title: string, dueDate: string) => {
    const date = new Date(dueDate);
    const end = new Date(date);
    end.setHours(date.getHours() + 1);
    const stamp = (value: Date) => value.toISOString().replace(/[-:]/g, '').replace(/\.\d{3}/, '');
    const content = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'BEGIN:VEVENT', `UID:shelfly-${date.getTime()}@library`, `DTSTAMP:${stamp(new Date())}`, `DTSTART:${stamp(date)}`, `DTEND:${stamp(end)}`, `SUMMARY:Retour du livre : ${title}`, 'DESCRIPTION:Rappel Shelfly : pensez à retourner ou renouveler votre emprunt.', 'END:VEVENT', 'END:VCALENDAR'].join('\r\n');
    const url = URL.createObjectURL(new Blob([content], { type: 'text/calendar;charset=utf-8' }));
    const link = document.createElement('a');
    link.href = url;
    link.download = `rappel-retour-${date.toISOString().slice(0, 10)}.ics`;
    link.click();
    URL.revokeObjectURL(url);
  };
  const downloadQrCode = () => {
    if (!portal.qrCode?.startsWith('data:image/')) return;
    const link = document.createElement('a');
    link.href = portal.qrCode;
    link.download = `shelfly-carte-${portal.matricule}.png`;
    link.click();
  };
  ...
  if (portal) {
    return (
      <div className="space-y-6">
        ...
        <Card><CardContent className="flex flex-col items-center gap-3 py-5 text-center"><p className="font-medium">Mon QR Code d’adhérent</p>{portal.qrCode?.startsWith('data:image/') ? <img src={portal.qrCode} alt="QR Code adhérent" className="size-36 rounded-md border bg-white p-2" /> : <p className="text-sm text-muted-foreground">Générez votre code pour le présenter au comptoir.</p>}<div className="flex flex-wrap justify-center gap-2"><Button size="sm" onClick={() => qrMutation.mutate()} isLoading={qrMutation.isPending}>{portal.qrCode?.startsWith('data:image/') ? 'Actualiser mon QR Code' : 'Générer mon QR Code'}</Button><Button size="sm" variant="outline" onClick={() => downloadCardMutation.mutate()} isLoading={downloadCardMutation.isPending}>Télécharger ma carte</Button></div></CardContent></Card>
        ...
        <Card><CardContent className="py-5"><div className="flex items-center justify-between gap-3"><div><h2 className="font-display font-semibold">Nouveautés du catalogue</h2><p className="text-sm text-muted-foreground">Les derniers ouvrages ajoutés à Shelfly.</p></div><Button asChild size="sm" variant="outline"><a href="/catalogue">Voir le catalogue</a></Button></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{latestBooksQuery.data?.items.map((book) => <a key={book.id} href="/catalogue" className="rounded-md border p-3 transition-colors hover:bg-muted"><p className="truncate text-sm font-medium">{book.title}</p><p className="mt-1 truncate text-xs text-muted-foreground">{book.authors.map((item) => item.author.name).join(', ') || 'Auteur inconnu'}</p>{(book.digitalFileUrl || book.externalLink) && <p className="mt-2 text-xs font-medium text-primary">Lecture numérique</p>}</a>) ?? <p className="text-sm text-muted-foreground">Chargement des nouveautés…</p>}</div></CardContent></Card>
        {portal.borrows.filter((borrow) => ['ONGOING', 'RENEWED'].includes(borrow.status)).length > 0 && <Card><CardContent className="py-5"><h2 className="font-display font-semibold">Rappels de retour</h2><p className="mt-1 text-sm text-muted-foreground">Ajoutez vos échéances à votre calendrier pour ne pas les oublier.</p><div className="mt-4 space-y-2">{portal.borrows.filter((borrow) => ['ONGOING', 'RENEWED'].includes(borrow.status)).map((borrow) => <div key={borrow.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border p-3"><span className="text-sm">{borrow.bookCopy.book.title} · retour le {formatDate(borrow.dueDate)}</span><Button size="sm" variant="outline" onClick={() => addDueDateToCalendar(borrow.bookCopy.book.title, borrow.dueDate)}>Ajouter au calendrier</Button></div>)}</div></CardContent></Card>}
        {portal.qrCode?.startsWith('data:image/') && <Card><CardContent className="flex flex-wrap items-center justify-between gap-3 py-4"><div><p className="font-medium">QR Code dans mon téléphone</p><p className="text-sm text-muted-foreground">Enregistrez-le pour le présenter rapidement au comptoir.</p></div><Button variant="outline" size="sm" onClick={downloadQrCode}>Télécharger mon QR Code</Button></CardContent></Card>}
